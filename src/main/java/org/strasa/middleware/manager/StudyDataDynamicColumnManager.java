...
import org.strasa.middleware.model.custom.StudyDataDynamicColumnQueryBuilder.Queries;
...
public class StudyDataDynamicColumnManager {
	...
	public void deleteStudyDataset(Integer dataset) {
		String sql = "DELETE FROM " + ((isRaw) ? "studyrawdatadynacol" : "studyderivedatadynacol") + " WHERE dataset = " + dataset;
		try {
	...
	public void deleteStudyData(Integer studyID) {
		String sql = "DELETE FROM " + ((isRaw) ? "studyrawdatadynacol" : "studyderivedatadynacol") + " WHERE studyid = " + studyID;
		try {
			java.sql.PreparedStatement ps = con.prepareStatement(sql);
			ps.execute();
		} catch (SQLException e) {
	...
	public void mergeDatasetFromStudy(Integer studyID) {
		StudyDataSetManagerImpl datasetMan = new StudyDataSetManagerImpl();
		List<StudyDataSet> lstDataset = datasetMan.getDataSetsByStudyId(studyID, ((isRaw) ? "rd" : "dd"));

		if (lstDataset.isEmpty())
			return;

		Integer latestDatasetID = lstDataset.get(0).getId();

		String sql = "UPDATE " + ((isRaw) ? "studyrawdatadynacol" : "studyderivedatadynacol") + " SET dataset = " + latestDatasetID + " WHERE studyid = " + studyID;
	...
	public ArrayList<ArrayList<String>> getStudyRawDataDynaColsWithWhere(Integer studyid, Integer dataset, Integer start, Integer limit, StudyDataDynamicColumnQueryBuilder sqlBuilder) {
		...
		try {
			...
			List<StudyDataColumn> lstColumns = new StudyDataColumnManagerImpl().getStudyDataColumnByStudyId(studyid, ((isRaw) ? "rd" : "dd"), dataset);
			String query = " Select * from ( SELECT id, ";
			ArrayList<String> qs = new ArrayList<String>();
			for (StudyDataColumn col : lstColumns) {
				qs.add("COLUMN_GET(dynamic_cols, '" + col.getColumnheader() + "' as char) AS '" + col.getColumnheader() + "'");
			}
			...
			if (!sqlBuilder.arrQueries.isEmpty()) {
				for (Queries q : sqlBuilder.arrQueries) {
	...
	public ArrayList<AcrossStudyData> getAcrossStudyResult(StudyDataDynamicColumnQueryBuilder sqlBuilder) {
		...
		try {
			...
			while (rs.next()) {
				...
				across.setGname(rs.getString("GName"));

				across.setOtherdata(new ArrayList<String>());

				for (String col : sqlBuilder.getQueriesColumn()) {

					across.getOtherdata().add(rs.getString(col));
				}

				// for (int i = 0; i < sqlBuilder.getQueriesColumn().size();
				// i++) {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (AcrossQueryException e) {
			// TODO Auto-generated catch block
			Messagebox.show("No study is using these columns", "Warning", Messagebox.OK, Messagebox.EXCLAMATION);
		}
	...
	public Long countStudyDynamicColWithWhereClause(Integer studyid, Integer dataset, StudyDataDynamicColumnQueryBuilder sqlBuilder) {
		Long returnVal = new Long(0);
		...
		try {
			...
			if (!sqlBuilder.arrQueries.isEmpty()) {
				for (Queries q : sqlBuilder.arrQueries) {
					query = query + " AND " + "'" + q.column + "' " + q.operator + " '" + q.value + "'";
				}
			}
			query = query + ";";
