...
public class StudyGermplasmInfo extends ProcessTabViewModel {
	...
	public GermplasmGroupingModel getGermplasmModel() {
		...
		return new GermplasmGroupingModel(allData, new GermplasmComparator(),
				this.showGroup);
	}
	...
	public void selectGermplasm(
			@BindingParam("germplasm") GermplasmDeepInfoModel data) {
		...
		if (selectedGermplasm.isKnown()) {
			...
			BindUtils.postNotifyChange(null, null,
					lstKnownGermplasm.get(oldGerm), "styleBG");
		} else {

			lstStudyGermplasm.get(oldGerm).setStyleBG("background-color: #FFF");
			BindUtils.postNotifyChange(null, null,
					lstStudyGermplasm.get(oldGerm), "styleBG");
		}
		if (data.isKnown()) {
			lstKnownGermplasm.get(data.getGermplasmname()).setStyleBG(
					"background-color: #BEC7F7 ");
			BindUtils.postNotifyChange(null, null,
					lstKnownGermplasm.get(data.getGermplasmname()), "styleBG");
			System.out.println("COLOR CHANGED");
		} else {

			lstStudyGermplasm.get(data.getGermplasmname()).setStyleBG(
					"background-color: #BEC7F7 ");
			BindUtils.postNotifyChange(null, null,
					lstStudyGermplasm.get(data.getGermplasmname()), "styleBG");
			System.out.println("COLOR CHANGED");
		}
	...
	public void init(
		...
		List<Germplasm> lst = rawMan.getStudyGermplasmInfo(studyID,
				dataset.getId());
		...
		for (Germplasm germData : lst) {
			...
			if (germplasmMan.isGermplasmExisting(germData.getGermplasmname())) {
				...
				List<Germplasm> germplasmList = germplasmMan
						.getGermplasmListByName(germData.getGermplasmname());
				for (Germplasm subGermData : germplasmList) {
					...
					newData.setCharacteristicValues(germCharMan
							.getGermplasmByGermplasmName(germData
									.getGermplasmname()));
					newData.setSelectedGermplasmType(getGermplasmTypeById(newData
							.getGermplasmtypeid()));
					newData.setKnown(true);
					newData.setRowIndex(lstKnownGermplasm.size());

					// IF Record does not exist, add
					if (!lstKnownGermplasm.containsKey(newData
							.getGermplasmname())) {
						lstKnownGermplasm.put(newData.getGermplasmname(),
								newData);
					...
					if (newData.getUserid() == this.getUserID()) {
						...
						if (!lstKnownGermplasm.containsKey(newData
							...
							if (!lstKnownGermplasm.get(newData
									.getGermplasmname()).recordExist) {
								lstKnownGermplasm.put(
										newData.getGermplasmname(), newData);
							}
						}

					}
					// IF Record exist in the studgermplasm table
					if (studyGermMan.isGermplasmRecordExist(
							subGermData.getId(), this.studyID,
							this.dataset.getId())) {
	...
	@Command
	public void changeGermplasmInfo(
			@BindingParam("index") GermplasmDeepInfoModel selected) {
				...
				if (!StringUtils.isNullOrEmpty(germData.getGermplasmname())) {
					if (lstStudyGermplasm.containsKey(germData
						...
						lstStudyGermplasm.get(germData.getGermplasmname())
								.setGermplasmtypeid(
										getGermplasmTypeById(germData
												.getGermplasmtype()));
						lstStudyGermplasm.get(germData.getGermplasmname())
								.setValueFromeGermplasmEx(germData);
					}
				}
			}
	...
	public void updateCharacteristicInfo(
		...
		BindUtils.postNotifyChange(null, null,
				this.getLstStudyGermplasm().get(model.getRowIndex()), "*");
	...
	public boolean validateTab() {
		...
		studyGermplasmMan.addStudyGermplasmBatch(lstStudyGermpl, this.studyID,
				this.dataset.getId());
