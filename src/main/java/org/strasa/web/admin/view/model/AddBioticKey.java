...
public class AddBioticKey {
	public static String ZUL_PATH = "/admin/settings/addbiotickey.zul";
	private KeyBiotic model = new KeyBiotic();
	public KeyBiotic getModel() {
		return model;
	}

	public void setModel(KeyBiotic model) {
		this.model = model;
	}
	...
	public void add(){
		...
		if(man.getAllBioticAsString().contains(model.getValue())){
			Messagebox.show("Biotic key already exists! Choose a different name.", "OK", Messagebox.OK, Messagebox.EXCLAMATION);
			return;
		}
		try {
