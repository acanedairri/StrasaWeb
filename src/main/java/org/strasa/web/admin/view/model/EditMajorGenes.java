...
public class EditMajorGenes {
	...
	public void init(@ContextParam(ContextType.VIEW) Component view){
		...
		makeRowStatus(man.getAllMajorGenes());
	}

	private void makeRowStatus(List<KeyMajorGenes> list) {
		// TODO Auto-generated method stub

		rowList.clear();
		for (KeyMajorGenes p: list){
...
//		man.updateBiotic(ps.getValue());
	...
	public void delete(@BindingParam("id") final Integer Id, @BindingParam("keyName") String keyName){
		if(!germplasmCharMan.isCharacteristicValueExisting("MajorGenes", keyName)){
			Messagebox.show("Are you sure you want to delete "+keyName+"?",
					"Delete", Messagebox.OK | Messagebox.CANCEL,
					Messagebox.QUESTION, new EventListener() {
				public void onEvent(Event e) {
					if ("onOK".equals(e.getName())) {
						man.deleteMajorGenesById(Id);
						makeRowStatus(man.getAllMajorGenes());
						BindUtils.postNotifyChange(null, null,
								EditMajorGenes.this, "rowList");
						Messagebox.show("Changes saved.");
		...
		} else  Messagebox.show("Cannot delete a key that is in use.", "Error", Messagebox.OK, Messagebox.ERROR); 
	}

	//	@NotifyChange("list")
	@Command("add")
	public void add(@ContextParam(ContextType.COMPONENT) Component component) {
		Window win = (Window) component.getFellow("editMajorGenesWindow");
		Map<String, Object> params = new HashMap<String, Object>();

		params.put("oldVar", null);

		Window popup = (Window) Executions.createComponents(
				AddMajorGenes.ZUL_PATH, win, params);
	...
	public class RowStatus {
		private  KeyMajorGenes value;
		private boolean editingStatus;

		public RowStatus(KeyMajorGenes p, boolean editingStatus) {
			this.setValue(p);
		...
		public KeyMajorGenes getValue() {
			return value;
		}


		public void setValue(KeyMajorGenes p) {
			this.value = p;
		}
	}
}