...
public class AddAbioticKey {
	public static String ZUL_PATH = "/admin/settings/addabiotickey.zul";
	private KeyAbiotic model = new KeyAbiotic();
	public KeyAbiotic getModel() {
		return model;
	}

	public void setModel(KeyAbiotic model) {
		this.model = model;
	}
	...
	public void add(){
		...
		if(man.getAllAbioticAsString().contains(model.getValue())){
			Messagebox.show("Abiotic key already exists! Choose a different name.", "OK", Messagebox.OK, Messagebox.EXCLAMATION);
			return;
		}
		try {
